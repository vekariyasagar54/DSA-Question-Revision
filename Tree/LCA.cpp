...
MediumAccuracy: 52.85%Submissions: 158K+Points: 4
...
class Solution
{
    ...
    bool fun(Node* root , int n1 , int  n2 , Node* &ans)
    {
        ...
        bool left = fun(root->left , n1 , n2 , ans);
        if(left == 1 && self == 1)
        {
            if(!ans)
            {
        ...
        bool right = fun(root->right , n1 , n2 , ans);
        if(right == 1 && self == 1)
        {
            if(!ans)
            {
